...
class RemoveComplianceTilt:
    ...
    def plot_transfer_function(self, channels, **kwargs):
        ...
        if save_fig:
            name = label + "." + str(s.stats.starttime.julday) + "." + str(
                s.stats.starttime.year) + ".Transfer" + ".png"
            path = os.path.join(path_save, name)
            plt.savefig(path, dpi=150, format='png')
            plt.close()
        else:
    ...
    def plot_compare_spectrums(self, channels, nfft=15, noverlap=50, **kwargs):
        ...
        f, Zpownew = signal.welch(self.Znew.data, fs=self.fs, window='hamming', nperseg=nfft, noverlap=noverlap,
                                  nfft=nfft,
                                  detrend='linear', return_onesided=True, scaling='density', axis=-1)
        ##
...
if __name__ == "__main__":
    ...
    path_files_example = '/Users/robertocabiecesdiaz/Documents/tiskitpy/events_upflow/toy_problem/data'
    #
    file_z = read(os.path.join(path_files_example, 'WM.OBS05..SHZ.D.2015.258'))
    file_e = read(os.path.join(path_files_example, 'WM.OBS05..SHX.D.2015.258'))
    file_n = read(os.path.join(path_files_example, 'WM.OBS05..SHY.D.2015.258'))
    file_h = read(os.path.join(path_files_example, 'WM.OBS05..SDH.D.2015.258'))
    #
    tr_z = file_z[0]
    tr_e = file_e[0]
    tr_n = file_n[0]
